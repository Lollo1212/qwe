...
public class ClassAssignmentDetailsBackend implements GwtRpcImplementation<ClassAssignmentDetailsRequest, ClassAssignmentDetails> {
	...
	public ClassAssignmentDetails execute(ClassAssignmentDetailsRequest request, SessionContext context) {
		...
		if (solver != null) {
			ClassAssignmentDetails details = solver.getClassAssignmentDetails(cx, request.getClassId(), true);
	...
	public static ClassAssignmentDetails createClassAssignmentDetails(SuggestionsContext context, Solver solver, Lecture lecture, boolean includeConstraints) {
		return createClassAssignmentDetails(context, solver, lecture, (Placement)solver.currentSolution().getAssignment().getValue(lecture), includeConstraints);
	}
		
	public static ClassAssignmentDetails createClassAssignmentDetails(SuggestionsContext context, Solver solver, Lecture lecture, Placement placement, boolean includeConstraints) {
		...
		if (placement != null) {
			...
			details.setPreferences(toPreferenceInfo(new org.unitime.timetable.solver.ui.AssignmentPreferenceInfo(solver, placement)));
			for (Criterion<Lecture, Placement> criterion: lecture.getModel().getCriteria()) {
				...
				details.getPreferences().setCriterion(criterion.getName(), criterion.getValue(assignment, placement, null));
			}
		}
		...
		if (includeConstraints) {
			for (Iterator e=lecture.activeJenrls(assignment).iterator();e.hasNext();) {
				...
				if (!jenrl.isToBeIgnored())
					details.addStudentConflict(new StudentConflictInfo(toJenrlInfo(new org.unitime.timetable.solver.ui.JenrlInfo(solver, jenrl)), createClassAssignmentDetails(context, solver, another, false)));
			}
			if (placement!=null) {
			...
			for (Constraint c: lecture.constraints()) {
				if (c instanceof GroupConstraint) {
					...
					for (Lecture another: gc.variables()) {
						...
						dist.addClass(createClassAssignmentDetails(context, solver, another, false));
					}
			...
			if (!lecture.getInstructorConstraints().isEmpty() && placement!=null) {
				for (InstructorConstraint ic: lecture.getInstructorConstraints()) {
				    for (Lecture other: ic.variables()) {
				        ...
				        details.addBtbInstructorConflict(new BtbInstructorInfo(createClassAssignmentDetails(context, solver, other, false), pref));
				    }
				}
			}
		}
	...
	public static ClassAssignmentDetails createClassAssignmentDetailsFromAssignment(SuggestionsContext context, org.unitime.timetable.model.Assignment assignment, boolean includeConstraints) {
		...
		details.setPreferences(toPreferenceInfo(assignmentInfo));
	...
	public static AssignmentPreferenceInfo toPreferenceInfo(org.unitime.timetable.solver.ui.AssignmentPreferenceInfo info) {
		if (info == null) return null;
		AssignmentPreferenceInfo ret = new AssignmentPreferenceInfo();
		ret.setNormalizedTimePreference(info.getNormalizedTimePreference());
		ret.setBestNormalizedTimePreference(info.getBestNormalizedTimePreference());
		ret.setTimePreference(info.getTimePreference());
		ret.setRoomPreference(info.getRoomPreference());
		ret.setBestRoomPreference(info.getBestRoomPreference());
		ret.setNrStudentConflicts(info.getNrStudentConflicts());
		ret.setNrHardStudentConflicts(info.getNrHardStudentConflicts());
		ret.setNrDistanceStudentConflicts(info.getNrDistanceStudentConflicts());
		ret.setNrCommitedStudentConflicts(info.getNrCommitedStudentConflicts());
		ret.setNrTimeLocations(info.getNrTimeLocations());
		ret.setNrRoomLocations(info.getNrRoomLocations());
		ret.setNrSameRoomPlacementsNoConf(info.getNrSameRoomPlacementsNoConf());
		ret.setNrSameTimePlacementsNoConf(info.getNrSameTimePlacementsNoConf());
		ret.setNrPlacementsNoConf(info.getNrPlacementsNoConf());
		ret.setBtbInstructorPreference(info.getBtbInstructorPreference());
		ret.setIsInitial(info.getIsInitial());
		ret.setInitialAssignment(info.getInitialAssignment());
		ret.setHasInitialSameTime(info.getHasInitialSameTime());
		ret.setHasInitialSameRoom(info.getHasInitialSameRoom());
		ret.setPerturbationPenalty(info.getPerturbationPenalty());
		ret.setTooBigRoomPreference(info.getTooBigRoomPreference());
		ret.setMinRoomSize(info.getMinRoomSize());
		ret.setUselessHalfHours(info.getUselessHalfHours());
		ret.setDeptBalancPenalty(info.getDeptBalancPenalty());
		ret.setSpreadPenalty(info.getSpreadPenalty());
		ret.setMaxDeptBalancPenalty(info.getMaxDeptBalancPenalty());
		ret.setMaxSpreadPenalty(info.getMaxSpreadPenalty());
		ret.setGroupConstraintPref(info.getGroupConstraintPref());
		ret.setDatePatternPref(info.getDatePatternPref());
		ret.setStudentGroupPercent(info.getStudentGroupPercent());
		ret.setStudentGroupComment(info.getStudentGroupComment());
		return ret;
	}
